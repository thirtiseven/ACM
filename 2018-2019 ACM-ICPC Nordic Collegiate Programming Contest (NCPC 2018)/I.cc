...
public:
	...
	BigNum(const std::string);     //将一个字符串类型的变量转化为大数
...
BigNum::BigNum(const std::string s) {    //将一个字符串类型的变量转化为大数
	int t,k,index,l,i;
	memset(a,0,sizeof(a));
	l=s.length();
	len=l/dlen;
...
int n;
BigNum sum;
std::pair<BigNum, int> a[maxx];
std::string str[maxx];
std::string s;
std::vector<std::string> ans;

bool cmp(std::pair<BigNum, int> a, std::pair<BigNum, int> b) {
