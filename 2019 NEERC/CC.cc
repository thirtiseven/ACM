...
struct node {
	int l, r, num, val;
	node() {}
	node(int l, int r, int num, int val)
		:l(l), r(r), num(num), val(val) {}
...
int main() {
	...
	scanf("%d%d%d", &n, &k, &m);
	for (int i = 0; i < m; i++)
		scanf("%d%d%d%d", &no[i].l, &no[i].r, &no[i].num, &no[i].val);
	sort(no, no + m);
	build(1, 1, n);
	ll ans = 0;
	for (int i = 0; i < m; i++) {
		ql = no[i].l, qr = no[i].r, qt = no[i].num;
		ll tsum1 = querysum(1,1,n,ql,qr), tsum2;
		add(1,1,n,ql,qr,qt);
		qt = k;
		changemin(1,1,n,ql,qr,qt);
		tsum2 = querysum(1,1,n,ql,qr);
		ans += (tsum2 - tsum1)*ll(no[i].val);
	}
	printf("%lld\n", ans);
	return 0;
}