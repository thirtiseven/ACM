...
int main(int argc, char *argv[]) {
	...
	while (T--) {
		...
		for (int i = 1; i <= n; i++) {
			std::cin >> a[i];
			sum += a[i];
			mcc1.add_directed_edge(n+1, i, 0, a[i], 0);
			mcc2.add_directed_edge(n+1, i, 0, a[i], 0);
		}
		if (sum % n == 0) {
			int x = sum / n;
			for (int i = 1; i <= n; i++) {
				mcc1.add_directed_edge(i, n+2, 0, x, 0);
				mcc2.add_directed_edge(i, n+2, 0, x, 0);
			}
		} else {
			std::cout << "-1\n";
			continue;
		...
		for (int i = 0; i < m; i++) {
			std::cin >> u >> v >> w;
			mcc1.add_directed_edge(u, v, 0, 10000000, 0);
			mcc2.add_directed_edge(u, v, 0, 10000000, w);
		}
		mcc1.add_directed_edge(n+2, n+1, 0, 1e18, -1);
		auto max_flow = -mcc1.minimum_cost_circulation().second;
		mcc2.add_directed_edge(n+2, n+1, max_flow, max_flow, 0);
		if (max_flow != sum) {
			std::cout << "-1\n";
		} else {
			std::cout << mcc2.minimum_cost_circulation().second << '\n';
		}
	}
}